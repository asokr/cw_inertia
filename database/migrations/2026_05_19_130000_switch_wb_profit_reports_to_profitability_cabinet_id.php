    ...
    public function up(): void
    {
        ...
        if (!Schema::hasColumn('wb_profit_reports', 'profitability_cabinet_id')) {
            Schema::table('wb_profit_reports', function (Blueprint $table) {
                $table->unsignedBigInteger('profitability_cabinet_id')->nullable()->after('cabinet_id');
            });
        }

        DB::statement("UPDATE `wb_profit_reports`
            SET `profitability_cabinet_id` = CAST(JSON_UNQUOTE(JSON_EXTRACT(`result_json`, '$.meta.profitability_cabinet_id')) AS UNSIGNED)
            WHERE `profitability_cabinet_id` IS NULL");

        DB::statement("DELETE r
            FROM `wb_profit_reports` r
            LEFT JOIN `wb_profitability_cabinets` p ON p.id = r.profitability_cabinet_id
            WHERE r.profitability_cabinet_id IS NULL
               OR r.profitability_cabinet_id = 0
               OR p.id IS NULL");

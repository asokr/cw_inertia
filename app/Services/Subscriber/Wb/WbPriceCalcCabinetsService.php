...
class WbPriceCalcCabinetsService
{
    ...
    public function store(Request $request)
    {
        ...
        if (isset($limits['price_calc_clients']) && ! ToolLimits::canUsePlanLimit(auth()->user(), $limits, 'price_calc_clients')) {
            return response()->json(["success" => false, "messages" => ["Вы исчерпали лимит на количество кабинетов."]], 200);
        }

        $updatedLimits = isset($limits['price_calc_clients'])
            ? ToolLimits::applyPlanLimitConsumption(auth()->user(), $limits, 'price_calc_clients')
            : null;

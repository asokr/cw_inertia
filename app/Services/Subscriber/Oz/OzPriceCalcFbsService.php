...
class OzPriceCalcFbsService
{
    ...
    public function index(Request $request, int $cabinetId)
    {
        ...
        $query = OzPriceCalcFbs::where('cabinet_id', $cabinet->id);
        ...
        $items->setCollection(
            $items->getCollection()->map(static function (OzPriceCalcFbs $item): array {
                $row = $item->toArray();
        ...
        return response()->json([
            'success' => true,
            'messages' => ['Список номенклатур FBS'],
            'data' => $items,
            'columns' => $this->columnsPayload('fbs'),
        ], 200);
    }
    ...
    {
        $batchName = sprintf('ozon_fbs_export_%s', $cabinetId);
        $isExporting = $this->hasActiveBatch($batchName);

        $fileUrl = null;
        if (! $isExporting) {
            $path = "ozon/price-calc/{$cabinetId}/fbs.xlsx";
    ...
    public function export(Request $request, int $cabinetId)
    {
        ...
        $type = strtolower((string) $request->input('type', 'fbs'));

        if ($type !== 'fbs') {
            return response()->json([
                'success' => false,
                'messages' => ['Поддерживается только экспорт FBS'],
            ], 200);
        }

        $batchName = sprintf('ozon_fbs_export_%s', $cabinet->id);
        ...
        $batch = Bus::batch([
            new \App\Jobs\Ozon\ExportPriceCalcJob($cabinet->id, 'fbs'),
    ...
    {
        $batchName = sprintf('ozon_fbs_calculate_%s', $cabinetId);
    ...
    public function calculate(Request $request, int $cabinetId)
    {
        ...
        $batchName = sprintf('ozon_fbs_calculate_%s', $cabinet->id);
        ...
        $batch = Bus::batch([
            new \App\Jobs\Ozon\CalculatePriceJob($cabinet->id, 'fbs'),
        ])->name($batchName)->dispatch();
    ...
    {
        $batchName = sprintf('ozon_fbs_import_%s', $cabinetId);
    ...
    public function import(Request $request, int $cabinetId)
    {
        ...
        $batchName = sprintf('ozon_fbs_import_%s', $cabinet->id);
        ...
        $fileName = sprintf('ozon_fbs_import_%s_%s.xlsx', $cabinet->id, now()->timestamp);
        $filePath = $file->storeAs('imports', $fileName); // stores in storage/app/imports
        $absolutePath = \Illuminate\Support\Facades\Storage::path($filePath);

        $batch = Bus::batch([
            new \App\Jobs\Ozon\ImportPriceCalcJob($cabinet->id, 'fbs', $absolutePath),
        ])->name($batchName)->dispatch();
    ...
    public function sync(Request $request, int $cabinetId)
    {
        ...
        $batchName = sprintf('ozon_fbs_sync_%s', $cabinet->id);
        ...
        $batch = Bus::batch([
            new \App\Jobs\Ozon\SyncPriceCalcJob($cabinet->id, 'fbs'),
        ])->name($batchName)->dispatch();

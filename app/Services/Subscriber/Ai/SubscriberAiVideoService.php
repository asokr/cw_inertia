...
class SubscriberAiVideoService
{
    ...
    private function resolveGenerationUuidFromRequest(Request $request): ?string
    {
        if ($request->filled('generation_uuid')) {
            return (string) $request->input('generation_uuid');
        }

        if ($request->filled('generation_id')) {
            $generationId = (int) $request->input('generation_id');
            $subscriberId = (int) data_get($request->user(), 'subscriber.id');

            if ($generationId > 0 && $subscriberId > 0) {
                $generation = \App\Models\AiVideoGeneration::query()

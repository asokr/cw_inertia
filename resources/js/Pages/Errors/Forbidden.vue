            ...
            <main class="flex flex-1 flex-col items-center justify-center py-12">
                <div class="landing-page__fade landing-page__fade--1 mb-8 text-center">
                    <p class="mb-3 inline-flex items-center gap-2 rounded-full border bg-card/80 px-4 py-1.5 text-xs font-medium text-muted-foreground backdrop-blur">
                        <ShieldAlert class="h-3.5 w-3.5 text-primary" />
                        Ошибка 403
                    </p>
                    <div class="error-page__code select-none">403</div>
                </div>
                ...
                <Card class="landing-page__fade landing-page__fade--2 w-full max-w-2xl border-border/70 bg-card/90 p-8 shadow-xl shadow-primary/5 backdrop-blur sm:p-10">
                    <template v-if="authenticated">
                        <div class="mb-6 space-y-3 text-center sm:text-left">
                            <h1 class="text-2xl font-semibold tracking-tight sm:text-3xl">
                                Доступ запрещён{{ userName ? `, ${userName}` : "" }}
                            </h1>
                            <p class="text-sm leading-relaxed text-muted-foreground sm:text-base">
                                <template v-if="isSubscriber">
                                    Этот раздел недоступен для вашего тарифа или роли. Перейдите к доступным инструментам.
                                </template>
                                <template v-else-if="isAdmin">
                                    У вашей учётной записи нет прав на этот раздел. Вернитесь в панель управления или откройте инструменты подписчика.
                                </template>
                                <template v-else>
                        ...
                        <div v-if="isSubscriber || canAccessPanel" class="mt-8 border-t pt-6">
                    ...
                    <template v-else>
                        <div class="mb-6 space-y-3 text-center sm:text-left">
                            ...
                            <p class="text-sm leading-relaxed text-muted-foreground sm:text-base">
                                Для просмотра этой страницы нужна авторизация с подходящими правами. Войдите в аккаунт или зарегистрируйтесь, чтобы получить доступ к инструментам.
                            </p>
                        </div>
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row">
                            <Button as="a" href="/login" size="lg" class="w-full sm:flex-1">
                                <LogIn class="mr-2 h-4 w-4" />
                                Войти
                                <ArrowRight class="ml-2 h-4 w-4" />
                            </Button>
                            <Button as="a" href="/register" variant="outline" size="lg" class="w-full sm:w-auto">

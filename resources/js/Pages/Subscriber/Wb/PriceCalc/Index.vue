...
function submitAdd() {
    addForm.post("/panel/wb/price-calc/cabinets", {
        preserveScroll: true,
        onSuccess: () => {
            addOpen.value = false;
            addForm.reset();
...
function submitEdit() {
    editForm.put(`/panel/wb/price-calc/cabinets/${selectedCabinet.value.id}`, {
        preserveScroll: true,
        onSuccess: () => {
...
function confirmDelete() {
    router.delete(`/panel/wb/price-calc/cabinets/${selectedCabinet.value.id}`, {
        preserveScroll: true,
        onSuccess: () => {
...
<template>
    <Head title="Ценообразование Wildberries" />

    <SubscriberLayout title="Ценообразование Wildberries" :breadcrumbs="breadcrumbs">
        <ToolPageHeader
            title="Ценообразование Wildberries"
            :description="limits.price_calc_clients !== null ? `Доступно кабинетов по тарифу: ${limits.price_calc_clients}` : ''"
        >
            <template #actions>

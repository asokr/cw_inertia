    ...
    <Dialog :open="open" :title="title" :description="description" @update:open="$emit('update:open', $event)">
        <form class="space-y-4" @submit.prevent="submit">
            <div class="space-y-2">
                ...
                <Input :model-value="modelValue.name" placeholder="Мой кабинет" :error="Boolean(errors.name)"
            ...
            <div v-if="isOzon" class="space-y-2">
                ...
                <Input :model-value="modelValue.client_id" placeholder="Ozon Client ID"
                    :error="Boolean(errors.client_id)" @update:model-value="updateField('client_id', $event)" />
                <p v-if="errors.client_id" class="text-xs text-destructive">{{ errors.client_id }}</p>
            </div>

            <ApiKeyField :model-value="modelValue.apikey"
                :label="isOzon ? 'API-ключ Ozon (Seller API)' : 'API-ключ Wildberries'" :error="Boolean(errors.apikey)"
            ...
            <template v-if="isOzon">
                <div class="space-y-2 rounded-md border border-border/60 bg-muted/30 p-3">
                    ...
                    <ApiKeyField :model-value="modelValue.performance_client_secret ?? ''"
                        label="Performance Client Secret" :error="Boolean(errors.performance_client_secret)"
            ...
            <div v-if="!isOzon && modelValue.brands !== undefined" class="space-y-2">
                ...
                <Input :model-value="modelValue.brands" placeholder="Brand A, Brand B" :error="Boolean(errors.brands)"
